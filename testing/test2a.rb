...
def pangram sent
  sent.chars.uniq.reject{|x| x== ' '}.map(&:downcase).sort == ('a'..'z').to_a
...
def gem_element arr
  count, stones, freq = 0, arr.size, Hash.new(0)
...
def anagram string
  count, freq, mid = 0, Hash.new(0), string.size/2
...
# ====palindrome is a word that reads the same forwards and backwards===
# find the indices needs to make the string a palindrome

def palindrome_index string
  indices, s1 = [], string.chars
  if s1 == s1.reverse
    puts -1
  else
...
def equi_index arr
  left, right, indices = 0, arr.reduce(:+), []
  arr.each_with_index do |x,xi|
    right -= x
